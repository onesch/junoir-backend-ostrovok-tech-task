...
def parse_arguments() -> tuple[int, int, str]:
    ...
    if len(sys.argv) != 3:
        logger.error(
            "Invalid arguments, Usage: ./getwallpapers.py MMYYYY resolution"
        )
        sys.exit(1)
    ...
    if len(month_year) != 6 or not month_year.isdigit():
        raise ValueError("Date must be in MMYYYY format.")
...
def validate_arguments(year: int, month: int, resolution: str) -> None:
    ...
    if year == current_year and month > current_month:
        raise ValueError("Cannot request wallpapers from the future.")
    ...
    if not (width.isdigit() and height.isdigit()):
        raise ValueError("Resolution must contain only numbers.")
